  ...
  return new Promise((resolve, reject) => {
    try {
      if (cliInfo) {
        ...
        const isGlobal = fs.existsSync(gCLI) && path.dirname(require.resolve(path.join(gCLI, 'package.json'))) === path.dirname(__dirname);
        ...
        ['@enact/dev-utils', 'eslint-config-enact', 'eslint-plugin-enact', 'postcss-resolution-independence'].forEach((dep) =>
          logVersion(dep),
        );
        console.log();
