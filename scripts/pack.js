...
function details(err, stats, output) {
  ...
  if (messages.errors.length) {
  ...
  } else if (typeof process.env.CI === 'string' && process.env.CI.toLowerCase() !== 'false' && messages.warnings.length) {
    console.log(chalk.yellow('Treating warnings as errors because process.env.CI = true. ' + 'Most CI servers set it automatically.\n'));
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(chalk.yellow('NOTICE: This build contains debugging functionality and may run' + ' slower than in production mode.'));
    }
    console.log();
...
function cli(args) {
  const opts = minimist(args, {
    boolean: ['minify', 'framework', 'externals-corejs', 'stats', 'production', 'isomorphic', 'snapshot', 'verbose', 'watch', 'help'],
